...
After the first valid user message, the selected agent is locked for that chat. The Clear chat action removes the accumulated conversation and current-task memory, keeps durable long-term memory, and unlocks agent selection again. Agent Chat stores explicit memory layers separately: short-term memory holds successful current-dialog turns, working memory holds the active task goal/stage/plan/constraints/results, and long-term memory holds durable profile facts, preferences, decisions, reusable knowledge, and invariants. User messages can update memory through explicit lines such as `Goal: ...`, `Constraint: ...`, `Preference: ...`, `Decision: ...`, and `Invariant: ...`. Before a normal submit, a prompt builder chooses which layers to include, adds long-term and working memory as structured memory blocks when they are non-empty, appends short-term turns as dialogue history, and finally appends the latest user message. The screen shows concrete memory contents by layer, marks which layers were used by the last request, collapses long user/model messages until tapped, and renders inline Markdown such as bold and code spans instead of showing raw markers. If the active token budget is exceeded, the Gemini agent still applies a sliding window to the prepared request: it drops the oldest successful user/model turns from the request payload, keeps the latest user message, and caps `maxOutputTokens` to the remaining budget. Loading and error messages are shown in the chat, but they are not sent back as model context.
