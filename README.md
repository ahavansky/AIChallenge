...
After the first valid user message, the selected agent is locked for that chat. The Clear chat action removes only the accumulated conversation, so the chat history remains the short-term memory source and agent selection unlocks again. Agent Chat keeps explicit memory sources separate: short-term memory is selected from successful chat messages in the JSON history, working memory is an editable `TaskContext` with goal/stage/plan/constraints/open questions/results, and long-term memory is a user-editable `agent_chat_memory.md` file with durable profile facts, preferences, decisions, reusable knowledge, and invariants. Before a normal submit, a prompt builder chooses which sources to include, applies simple budget limits, adds long-term markdown and `TaskContext` blocks when they have content, appends the selected chat-history window, and finally appends the latest user message. The screen shows concrete memory contents by source, marks which sources were used by the last request, lets users edit `TaskContext` and save `agent_chat_memory.md`, collapses long user/model messages until tapped, and renders inline Markdown such as bold and code spans instead of showing raw markers. If the active token budget is exceeded, the Gemini agent still applies a sliding window to the prepared request: it drops the oldest successful user/model turns from the request payload, keeps the latest user message, and caps `maxOutputTokens` to the remaining budget. Loading and error messages are shown in the chat, but they are not sent back as model context.
